...
def hotcomments_mysql(html,artistname, songname, i, pages, total, mysql):
    print("正在获取歌曲{}的第{}页评论,总共有{}页{}条评论！\n".format(songname, i, pages, total))
    # 精彩评论
    m = 1
    # 键在字典中则返回True, 否则返回False
    if 'hotComments' in html:
        for item in html['hotComments']:
            ...
            # 写入文件
            print("热门评论{}: {} : {}    点赞次数: {}".format(m, user['nickname'], item['content'], item['likedCount']))
            mysql.insert('music163_comments',{'artistname':artistname,'songname':songname,'comment':item['content'],'support':str(item['likedCount'])})
...
def comments_mysql(html,artistname, songname, i, pages, total, mysql):
    print("\n正在获取歌曲{}的第{}页评论,总共有{}页{}条评论！\n".format(songname, i, pages, total))
    # 全部评论
    j = 1
    for item in html['comments']:
...
def read_csv(path='./files/songs.csv'):
    with open(path, "r", encoding="utf-8") as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            song_id, song_name,artist_name = row
            yield song_id, song_name,artist_name



def get_all_comments(mysql):

    for readcsv in read_csv():
        songid, songname,artistname = readcsv
        page = 1
        ...
        comments_mysql(html,artistname ,songname, page, pages, total,mysql)
...
def get_hot_comments(mysql):
    ...
    for readcsv in read_csv():
        ...
        # 总页数
        pages = math.ceil(total / 20)
        hotcomments_mysql(html,artistname ,songname, page, pages, total,mysql)


if __name__ == "__main__":
    mydb = MysqlHelper('127.0.0.1',3306,'root','123456','spider')
    # get_all_comments(mydb)
